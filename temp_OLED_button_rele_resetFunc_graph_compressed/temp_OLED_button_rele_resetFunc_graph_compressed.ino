...
void loop() {
  temp_update();
  TimeStringFunc(); //Has to be in the main loop to log the temperature, as the trigger for TempHistoryFunc() is inside TimeStringFunc() (Bad coding yes I know)
  if (current_temp > 60 && heatDelay == 0) {
    digitalWrite(rele, LOW);
    heatDelay = 60 * 15; //60*15 gives 15 minutes of delay with the delay(1000) at the bottom of the loop
  }
  else if (current_temp < 0 && heatDelay == 0) {
    digitalWrite(rele, LOW);
    error = true;
  }
...
String TimeStringFunc() {
  ...
  */
  ...
  if (((minutes % 12) == 0) && ((timeNow - timeSince) > 120000)) { //(minutes % 12 == 0 && timeNow - timeSince > 120000)
    timeSince = timeNow;
    TempHistoryFunc();
  }
...
void drawgraph() {
  uint8_t d_heigth = 64;
  int d_length = 128; //length of display
...
void TempHistoryFunc() {
  ...
  TempHistory[k] = current_temp;
  for (uint8_t j = 1; (j <= 8) && (k + j <= 127); j++) {
    TempHistory[k + j] = 70; //out of the picture
  }
  k ++;
}
